...
import { MapPin, Navigation, ArrowUpRight, Loader2, Info } from "lucide-react";
    ...
    const fetchNearby = async () => {
      ...
      try {
        const response = await fetch(
          `https://photon.komoot.io/api/?q=${encodeURIComponent(tagToSearch)}&lat=${userLocation.latitude}&lon=${userLocation.longitude}&limit=5`
        );
        const data = await response.json();
        
        const places: SuggestedPlace[] = data.features.map((f: any) => {
